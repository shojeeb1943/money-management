...
test('record-transaction rejects a category of the wrong kind', function (): void {
    [$user] = mcpCompany();
...
test('create-wallet and set-budget round-trip', function (): void {
    [$user, $company] = mcpCompany();
...
test('the mcp web endpoint requires a token', function (): void {
    $this->postJson('/mcp', [
        'jsonrpc' => '2.0',
        'id' => 1,
        'method' => 'ping',
...
test('the mcp web endpoint responds with a valid personal access token', function (): void {
    [$user] = mcpCompany();
...
test('oauth discovery endpoints are advertised for mcp clients', function (): void {
