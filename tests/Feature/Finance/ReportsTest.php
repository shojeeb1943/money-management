...
test('the income statement respects the period and rolls children into parents', function () {
    [$user, $company, $bank, , $commission, $hosting] = reportSetup();

    $vpsChild = app(CreateCategory::class)->handle($company, 'VPS', CategoryKind::Expense, $hosting->fresh());

    app(CreateTransaction::class)->handle($company, TransactionType::Income, $bank, 500_000, now(), $commission);
    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $bank, 100_000, now(), $hosting);
    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $bank, 50_000, now(), $vpsChild);

    app(CreateTransaction::class)->handle($company, TransactionType::Income, $bank, 999_999, now()->subMonths(2), $commission);
...
test('a voided transaction never appears in reports', function () {
    [$user, $company, $bank, , $commission] = reportSetup();

    $transaction = app(CreateTransaction::class)->handle($company, TransactionType::Income, $bank, 500_000, now(), $commission);
...
test('the balance sheet satisfies assets equal liabilities plus equity', function () {
    [$user, $company, $bank, $cash, $commission, $hosting] = reportSetup();

    app(CreateTransaction::class)->handle($company, TransactionType::CapitalInvestment, $bank, 2_000_000, now()->subMonth());
    app(CreateTransaction::class)->handle($company, TransactionType::Income, $bank, 800_000, now(), $commission);
    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $cash, 150_000, now(), $hosting);
    app(CreateTransfer::class)->handle($company, $bank, $cash, 300_000, now());
    app(CreateTransaction::class)->handle($company, TransactionType::CapitalWithdrawal, $bank, 250_000, now());

    $report = app(BalanceSheetReport::class)->generate($company, now());
    ...
    $historical = app(BalanceSheetReport::class)->generate($company, now()->subMonth()->endOfMonth());
...
test('the cash flow net change equals the wallet balance delta and classifies financing', function () {
    [$user, $company, $bank, $cash, $commission, $hosting] = reportSetup();

    app(CreateTransaction::class)->handle($company, TransactionType::CapitalInvestment, $bank, 1_000_000, now()->subMonths(2));

    $balancesBefore = $company->wallets()->sum('cached_balance');

    app(CreateTransaction::class)->handle($company, TransactionType::Income, $bank, 600_000, now(), $commission);
    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $cash, 100_000, now(), $hosting);
    app(CreateTransfer::class)->handle($company, $bank, $cash, 200_000, now());
    app(CreateTransaction::class)->handle($company, TransactionType::CapitalWithdrawal, $bank, 150_000, now());

    $balancesAfter = $company->wallets()->sum('cached_balance');

    $report = app(CashFlowReport::class)->generate($company, now()->startOfMonth(), now()->endOfMonth());
...
test('the report pages render', function () {
...
test('the category breakdown reports share per category', function () {
    [$user, $company, $bank, , $commission, $hosting] = reportSetup();

    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $bank, 75_000, now(), $hosting);
    $marketing = $company->categories()->where('name', 'Marketing')->firstOrFail();
    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $bank, 25_000, now(), $marketing);
...
test('the monthly summary covers twelve months and sums correctly', function () {
    [$user, $company, $bank, , $commission, $hosting] = reportSetup();

    app(CreateTransaction::class)->handle($company, TransactionType::Income, $bank, 500_000, now(), $commission);
    app(CreateTransaction::class)->handle($company, TransactionType::Expense, $bank, 200_000, now()->subMonthsNoOverflow(2), $hosting);
...
test('the dashboard stats respect the period filter', function () {
    [$user, $company, $bank, , $commission] = reportSetup();
